...
- ジャーナル本文、写真コピー、位置サンプルは端末内の Application Support 配下に保存する
- ジャーナリング中は、最近の回答、項目状態、写真枚数、位置サンプル数、任意の予定表要約を接続先サーバーへ journal prompt context として送る
- vision モードは有効時のみ、直近のカメラフレームを接続先サーバーへ送る
